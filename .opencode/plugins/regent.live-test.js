...
test('[LIVE] verify — all requirements met with matching vocabulary', async () => {
  ...
  const result = JSON.parse(await plugin.tool.verify.execute({
    requirements: ['Add user login form', 'Validate email format', 'Handle error states'].join('\n'),
    implementation_context: 'Created login form with user login form, validate email format implementation, handle error states on submit',
  }));
...
test('[LIVE] verify — some requirements unmet', async () => {
  const plugin = await RegentPlugin({ client: {} });
  const result = JSON.parse(await plugin.tool.verify.execute({
    requirements: ['Add user login form', 'Implement payment processing', 'Send confirmation email'].join('\n'),
...
test('[LIVE] verify — markdown checkbox lines handled', async () => {
  const plugin = await RegentPlugin({ client: {} });
  const result = JSON.parse(await plugin.tool.verify.execute({
    requirements: ['- [ ] Add user login', '- [x] Add API endpoint', '* [ ] Add test coverage'].join('\n'),
    implementation_context: 'Added user login form, added API endpoint route, added test coverage suite',
  }));
...
test('[LIVE] verify — detects YAGNI extras on separate lines', async () => {
  ...
  const result = JSON.parse(await plugin.tool.verify.execute({
    ...
    implementation_context: ['Added login form component with validation', 'Added admin dashboard with charts'].join('\n'),
  }));
...
test('[LIVE] config — registers all 6 commands properly', async () => {
  ...
  for (const name of ['orchestrate', 'delegate', 'research', 'tdd', 'diagnose', 'verify']) {
    ...
    assert.match(config.command[name].template, /\$ARGUMENTS/, `${name} missing $ARGUMENTS placeholder`);
  }
});
...
test('[LIVE] config — deduplicates against user commands', async () => {
  const plugin = await RegentPlugin({ client: {} });
  const config = {
    command: { tdd: { description: 'custom tdd', template: 'custom template', agent: 'custom-agent' } },
  };
  await plugin.config(config);
...
test('[LIVE] config — deduplicates against user agents', async () => {
  const plugin = await RegentPlugin({ client: {} });
  const config = {
    agent: { 'regent-general': { description: 'custom', mode: 'subagent', prompt: 'custom prompt' } },
  };
  await plugin.config(config);
...
test('[LIVE] bootstrap — injects EXTREMELY_IMPORTANT marker into first user message', async () => {
  const plugin = await RegentPlugin({ client: {} });
  const output = { messages: [{ info: { role: 'user' }, parts: [{ type: 'text', text: 'Do work' }] }] };
...
test('[LIVE] bootstrap — no inject when no user messages', async () => {
  const plugin = await RegentPlugin({ client: {} });
  for (const msgs of [{ messages: [] }, { messages: [{ info: { role: 'assistant' }, parts: [{ type: 'text', text: 'Hi' }] }] }]) {
    await plugin['experimental.chat.messages.transform']({}, msgs);
    const allText = msgs.messages.flatMap(m => m.parts || []).filter(p => p.type === 'text').map(p => p.text).join('');
...
test('[LIVE] bootstrap — prepends not appends', async () => {
  ...
  const output = { messages: [{ info: { role: 'user' }, parts: [{ type: 'text', text: userMsg }] }] };
  await plugin['experimental.chat.messages.transform']({}, output);
...
test('[LIVE] CONSTITUTION.md exists with all required content', () => {
  ...
  for (const role of ['Strategist', 'Architect', 'Fleet Commander', 'Inspector', 'Publisher', 'Mentor']) {
...
test('[LIVE] delegate_many handles empty tasks', async () => {
  const plugin = await RegentPlugin({
    client: {
      session: {
        async create() { return { data: { id: 's-e' } }; },
        async prompt() { return { data: { parts: [{ type: 'text', text: 'ok' }] } }; },
        async delete() { return { data: {} }; },
      },
    },
  });
  const o = JSON.parse(await plugin.tool.delegate_many.execute({ tasks: [] }, { directory: repoRoot }));
...
test('[LIVE] research handles empty questions', async () => {
  const plugin = await RegentPlugin({
    client: {
      session: {
        async create() { return { data: { id: 's-er' } }; },
        async prompt() { return { data: { parts: [{ type: 'text', text: 'ok' }] } }; },
        async delete() { return { data: {} }; },
      },
    },
  });
  const o = JSON.parse(await plugin.tool.research.execute({ questions: [] }, { directory: repoRoot }));
...
test('[LIVE] delegate — subagent creation failure is graceful', async () => {
  const plugin = await RegentPlugin({
    client: {
      session: {
        async create() { throw new Error('API unavailable'); },
      },
    },
  });
  const o = JSON.parse(await plugin.tool.delegate.execute(
...
test('[LIVE] dispatch — BLOCKED status detection', async () => {
  const plugin = await RegentPlugin({
    client: {
      session: {
        async create() { return { data: { id: 's-b' } }; },
        async prompt() { return { data: { parts: [{ type: 'text', text: 'BLOCKED\nCannot proceed' }] } }; },
        async delete() { return { data: {} }; },
      },
    },
  });
...
test('[LIVE] dispatch — NEEDS_CONTEXT status detection', async () => {
  const plugin = await RegentPlugin({
    client: {
      session: {
        async create() { return { data: { id: 's-n' } }; },
        async prompt() { return { data: { parts: [{ type: 'text', text: 'NEEDS_CONTEXT\nNeed endpoint URL' }] } }; },
        async delete() { return { data: {} }; },
      },
    },
  });
...
test('[LIVE] dispatch — CONCERN status detection', async () => {
  const plugin = await RegentPlugin({
    client: {
      session: {
        async create() { return { data: { id: 's-c' } }; },
        async prompt() { return { data: { parts: [{ type: 'text', text: 'Done\nCONCERN: Performance may degrade' }] } }; },
        async delete() { return { data: {} }; },
      },
    },
  });
...
test('[LIVE] dispatch — session cleanup on prompt failure', async () => {
  ...
  const plugin = await RegentPlugin({
    client: {
      session: {
        async create() { return { data: { id: 's-leak' } }; },
        async prompt() { throw new Error('fail'); },
        async delete(input) { deletedId = input.path.id; return { data: {} }; },
      },
    },
  });
  const o = JSON.parse(await plugin.tool.delegate.execute(
...
test('[LIVE] filesChanged — excludes numbered list items', async () => {
  const plugin = await RegentPlugin({
    client: {
      session: {
        async create() { return { data: { id: 's-fc' } }; },
        async prompt() { return { data: { parts: [{ type: 'text', text: ['## Summary', '1. Fixed login crash', '2. Added validation', '', 'src/index.js', 'package.json'].join('\n') }] } }; },
        async delete() { return { data: {} }; },
      },
    },
  });
...
test('[LIVE] filesChanged — matches extensionless paths with separators', async () => {
  const plugin = await RegentPlugin({
    client: {
      session: {
        async create() { return { data: { id: 's-f2' } }; },
        async prompt() { return { data: { parts: [{ type: 'text', text: ['### Files', 'src/components/Header', 'src/index.js', 'Makefile'].join('\n') }] } }; },
        async delete() { return { data: {} }; },
      },
    },
  });
...
test('[LIVE] research synthesis includes addressed findings', async () => {
  const plugin = await RegentPlugin({
    client: {
      session: {
        async create() { return { data: { id: 's-syn' } }; },
        async prompt() { return { data: { parts: [{ type: 'text', text: 'Findings' }] } }; },
        async delete() { return { data: {} }; },
      },
    },
  });
  const o = JSON.parse(await plugin.tool.research.execute(
...
test('[LIVE] research synthesis reports blocked questions', async () => {
  const plugin = await RegentPlugin({
    client: {
      session: {
        async create() { return { data: { id: 's-br' } }; },
        async prompt() { return { data: { parts: [{ type: 'text', text: 'BLOCKED\nNo access' }] } }; },
        async delete() { return { data: {} }; },
      },
    },
